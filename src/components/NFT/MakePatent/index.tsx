...
export const MakePatent = ({
  ...
  const { _assetPrice, _ipfsHash } = useFetchNFTAsset(id);
  ...
  const { data: tokenOwnerData, loading: tokenOwnerLoading } =
    ...
    const {data:otherUserAssetsData,loading:otherUserAssetsLoading} =  useGetAssetsByUser(owner)

  if (loading || !myNFTDetail || tokenIdByIndexLoading || tokenOwnerLoading || otherUserAssetsLoading)
    return <></>;
      ...
      <div className="container">
        ...
        <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5">
          {otherUserAssets.map((asset, index) => {
             return  <OtherAssetsByUser userAsset={asset} key={index} tokenOwner={tokenOwner} owner={owner}/>
